...
expiry_time = datetime.strptime('''2024-1-22 00:00:00.000''', '''%Y-%m-%d %H:%M:%S.%f''')
		...
		with requests.Session() as rsn:
			try:
				...
				if '"access_token":' in str(response.headers):
					...
					print('cookies ON : رست ✅')
				else:
					print("cookies NO : كوكيز مو شغال")
			except:
				print('####')
		exit()
	except Exception as e:
...
def menu(sy2,sy3):
	...
	print(('—'*25)+'\n• DeCoDe By @H_S_W_M •\n'+('—'*25))
	print('TELE : @ZZKGZ | @CP_OK2')
	print('\033[2;32--------------------------------------------------------------------------------------------{F}')
	print('')
	print('\x1b[38;5;208m 1 >  صيد من ايديات ✅ ')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s CP_OK2 %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
